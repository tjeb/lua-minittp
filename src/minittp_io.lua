...
function mio.read_fd_line(fd, strip_newline, timeout)
  ...
  local result = ""
  while true do
    local c, err, code = posix.read(fd, 1)
    if c == nil then return c, err, code end
...
function subprocess:read_line(strip_newline, timeout)
  ...
  local result, err, code = mio.read_fd_line(self.stdout, strip_newline, timeout)
  return result, err, code
end
...
function filereader:read_line(strip_newline)
  local result, err, code = mio.read_fd_line(self.fd, strip_newline)
